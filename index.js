...
const { limitend, limitcount } = require('./scr/limit')
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			const isPrem = userpremium.includes(sender)
                        ...
                        const NomerOwner = '5511949545217@s.whatsapp.net'
                        /******ApiKey Input******/
			...
			tels = body.slice(6)
               ...
               if (data.error) return reply(data.error)
                ...
                case 'imagem':
					...
					reply(`*agoea vc e premium ${addpremium} vc agora tem comandos premium*\n\nvc agora tem privilegios no bot:)`)
					break
				case 'removeprem':
					if (!isOwner) return reply(mess.only.ownerB)
					rprem = body.slice(13)
            ...
            break
				...
				case 'nsfwass':
						try {
							if (!isNsfw) return reply('❌ *NSFW Desativado* ❌')
                            if (!isUser) return reply(mess.only.daftarB)
                            if (isBanned) return reply(mess.only.benned)s.only.benned)
