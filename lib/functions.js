...
const wait = async (media) => new Promise(async (resolve, reject) => {
    ...
    if (!response.ok) reject(`Nachei a imagem não mano!`);
...
~> Season  : *${season}*`});
		...
		console.log(e)
		reject(`Sei que anime é esse não`)
	}
})
...
const start = (id, text) => {
	spins.add(id, {text: text})
		/*setTimeout() => {
			spins.succeed('load-spin', {text: 'Sucesso'})
		}, Number(wait) * 1000)*/
	}
const info = (id, text) => {
...
const banner = cfonts.render(('TIO|PAIN|BOT'), {
    font: 'chrome',
    color: 'candy',
    align: 'center',
    gradient: ["red","yellow"],
